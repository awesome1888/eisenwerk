...
module.exports.resetPwdWithShortToken = function(
    options,
    resetShortToken,
    identifyUser,
    password,
...
function resetPassword(options, query, tokens, password) {
    ...
    if (tokens.resetToken) {
        ...
        return Promise.reject(
            new errors.BadRequest('resetToken or resetShortToken is missing'),
        );
    }

    return Promise.all([userPromise, hashPassword(options.app, password)])
        .then(function(_ref) {
            ...
            Object.keys(tokens).forEach(function(key) {
                promises.push(
                    comparePasswords(tokens[key], user[key], function() {
